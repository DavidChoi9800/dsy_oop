...
class War(object):
    ...
    def war(self):
        print "How many card to be drawn?"
        n = int(input())
    ...
    def display_receive(self, player):
        if self.human:
            ...
            print "%s has %d cards and %s has %d cards" % \
                (player.name, len(player.hand)+len(player.discard), \
                self.player2.name, len(self.player2.hand)+len(self.player2.discard))
